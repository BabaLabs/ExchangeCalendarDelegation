...
do {
    $TargetMailbox = Read-Host "Please enter the email address of the mailbox that is BEING SHARED"` #Prompts the user for the email address being shared
    ...
    $DelegateAccount = Read-Host "Please enter the email address of the user GETTING ACCESS to the mailbox"` #Prompts the user for the email address that is getting access

    Write-Host 'Press any key to continue...'; `
        $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown'); #Another confirmation prompt, and opportunity to easily close the script

    Add-MailboxPermission -Identity $TargetMailbox -User $DelegateAccount -AccessRights FullAccess -InheritanceType All -Automapping $False #Runs the following command based on the input provided by the user.

    Write-Host 'Press any key to continue...'; `
        $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown'); #Another confirmation prompt, and opportunity to easily close the script

    Write-Output 'Outlook Auto-Mapping for mailbox '${TargetMailbox}' has been disabled for '${DelegateAccount}''`} #Visual confirmation for the user that the permissions have been added.
